  ...
  if (hasTable) {
    ...
    if (tableStart >= 0) {
      ...
      tableHtml = `<table class="w-full border-collapse my-2 text-sm"><thead><tr>${headers
        .map((h) => `<th class="px-2 py-1.5 bg-primary-50 border border-border text-left font-medium">${h}</th>`)
        .join('')}</tr></thead><tbody>${rows
        .map((row) => {
          const cells = row.split('|').filter(Boolean).map((c) => c.trim());
          return `<tr>${cells.map((c) => `<td class="px-2 py-1 border border-border">${c}</td>`).join('')}</tr>`;
        })
        .join('')}</tbody></table>`;
    }
  }
    ...
    <div className="relative group">
      ...
      {onSpeak && (
        <button
          ...
          className="absolute -bottom-1 -right-1 w-6 h-6 bg-gray-100 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
          title="朗读"
        >
...
export function MessageBubble({ message, onSpeak }: MessageBubbleProps) {
  ...
  return (
    <div className="flex gap-2 mb-3 animate-slide-in-left px-4">

  ...
  return (
    <div className="bg-white rounded-[20px] border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="p-3 space-y-3">
          ...
          {Object.values(metrics).slice(0, 3).map((metric, i) => {
            ...
            return (
              <div key={i} className="text-center bg-gray-50 rounded-lg p-2">
                <p className="text-[13px] text-text-secondary">{metric.label}</p>
          ...
          {Object.values(metrics).slice(3).map((metric, i) => {
            ...
            return (
              <div key={i} className="bg-gray-50 rounded-lg p-2 flex items-center justify-between">
                ...
                <span className={`text-[15px] font-bold ${rate >= 80 ? 'text-blue-600' : 'text-purple-500'}`}>
                  {metric.label === '保费收入'
                    ? `${(metric.actual / 10000).toFixed(1)}万`
        ...
        {/* Highlights */}
        <div>
          <p className="text-[13px] font-medium text-blue-700 mb-1">✅ 本周亮点：</p>
          {highlights.map((h, i) => (
            <p key={i} className="text-[13px] text-text-secondary ml-2 mb-0.5">• {h}</p>
          ))}
        </div>

        {/* Improvements */}
        <div>
          <p className="text-[13px] font-medium text-violet-700 mb-1">⚠️ 待改进：</p>
          {improvements.map((imp, i) => (
            <p key={i} className="text-[13px] text-text-secondary ml-2 mb-0.5">• {imp}</p>
          ))}
        </div>
      </div>
    </div>
